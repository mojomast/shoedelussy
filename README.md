...
- a local path-aware proxy that serves the built SPA and forwards `/api/*` to the worker on `:8788` (`LIVE_WORKER_PORT` overrides this; the live scripts default to `:8790`). `LIVE_PROXY_TIMEOUT` (default `120`s) controls the upstream socket timeout for slow LLM calls.
